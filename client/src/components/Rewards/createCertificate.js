...
function CreateCertificate() {
    ...
    useEffect(() => { 
        window.addEventListener('message', function(e) {
            if (e.origin === "https://embed.certificates.design" && typeof (e.data) === 'object') {
              console.log('Completed:', e.data.completed);
              console.log('Status:', e.data.status);
              console.log('Id:', e.data.design.id);
              console.log('Rasterized Content Url:', e.data.design.rasterized_content_url);
                    ...
                    await axios
                      .post(
                        process.env.NODE_ENV == 'production'
                          ? `${process.env.REACT_APP_API_SERVER}/api/certificate`
            ...
            async function updateGroup(){
              ...
              try {
                await axios.put(
                  `https://api.accredible.com/v1/issuer/groups/${groupid}`,creategroupdata,apicall)
                .then((response) => {
